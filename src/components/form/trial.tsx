...
export const TrialForm = (props: TTrialForm): JSX.Element => {
  ...
  const { lastName, firstName, phoneNumber, companyName, emailAddress, buttonSubmit, successMessage } = fields;
  ...
  return (
    <>
      <Flex as="form" onSubmit={handleSubmit(submit)} flexDir="column" w="100%">
        <FormProvider {...form}>
          <FieldGroup>
            <TextInput ctl={control} id={firstName.id} isRequired={firstName.required} placeholder={firstName.displayName} />
            <TextInput ctl={control} id={lastName.id} isRequired={lastName.required} placeholder={lastName.displayName} />
          </FieldGroup>
          <FieldGroup>
            <TextInput ctl={control} id={companyName.id} isRequired={companyName.required} placeholder={companyName.displayName} />
          </FieldGroup>
          <FieldGroup>
            <TextInput ctl={control} id={emailAddress.id} isRequired={emailAddress.required} placeholder={emailAddress.displayName} />
          </FieldGroup>
          <FieldGroup>
            <TextInput ctl={control} id={phoneNumber.id} isRequired={phoneNumber.required} placeholder={phoneNumber.displayName} />
          </FieldGroup>
          <Center px={2} mt={4} w="100%">
            <Button w="100%" type="submit" variant="outline" colorScheme="primary" isLoading={formState.isSubmitting}>
              {buttonSubmit}
            </Button>
          </Center>
        </FormProvider>
