...
const meteorOrbitAnimation = props => keyframes`
  0% {
    transform: translate(${`${-props.radius}px`}, 0) rotate3d(0, 0, 1, ${
    props.reverse ? "360deg" : "270deg"
});
  }
  100% {
    transform: translate(${`${-props.radius}px`}, 0) rotate3d(0, 0, 1, ${
    props.reverse ? "270deg" : "360deg"
});
  }
`;
...
const MeteorShape = styled.circle`
    ...
    ${({ animationReset }) =>
        animationReset !== true
            ? css`
                  animation: 5s ${meteorOrbitAnimation} 1 linear both,
                      5s ${meteorGlow} 1 cubic-bezier(0.175, 0.885, 0.32, 1.275)
                          both;
              `
            : css`
                  opacity: 0;
